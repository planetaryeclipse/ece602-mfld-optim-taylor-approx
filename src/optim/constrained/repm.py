...
class SubproblemLSE(MfldFunc):
    ...
    def value(self, p: torch.Tensor, cfg: ComputeMfld, *args: *FuncArgs) -> torch.Tensor:
        ...
        for h in self._hs:
            result += self._penalty * self._u * torch.log(torch.exp(h.value(p, cfg, *args) / self._u) + torch.exp(-h.value(p, cfg, *args) / self._u))
        return result
...
class SubproblemLQH(MfldFunc):
    ...
    def hess(self, p: torch.Tensor, cfg: ComputeMfld, *args: *FuncArgs) -> torch.Tensor:
        ...
        for h in self._hs:
            ...
            result += -self._penalty / torch.float_power(h_val**2 + self._u**2, 1.5/2) * h_val**2 * torch.outer(h_diff, h_diff)
...
@dataclass
class RepmCfg:
    acc_tol_min: float  # epsilon
    acc_tol_0: float
    acc_decay: float # (0, 1)

    penalty_0: float  # rho
    penalty_growth: float  # > 1
    penalty_update: bool  # whether to update the penalty
    penalty_constr_threshold: float # update if contraint violates this threshold

    approx_acc_min: float
    approx_acc_0: float  # u
    approx_acc_decay: float  # (0, 1)
...
def repm(f: MfldFunc,
    ...
    if mode == RepmMode.LSE:
        q_subproblem = SubproblemLSE(f, gs, hs, approx_accuracy, penalty)
    elif mode == RepmMode.LQH:
        q_subproblem = SubproblemLQH(f, gs, hs, approx_accuracy, penalty)
    ...
    for idx in range(cfg.max_iters):
        ...
        if cfg.penalty_update and (idx == 0 or torch.max(torch.tensor(next_g_vals).abs()) >= cfg.penalty_constr_threshold or torch.max(torch.tensor(next_h_vals).abs()) >= cfg.penalty_growth):
