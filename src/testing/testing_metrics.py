...
def scaled_metric(x1, x2, x3, scaling: float):
    ...
    metric[0, 0] = (x1 / scaling) ** 2 + 1.  # constant prevents degeneracy at origin
    metric[1, 1] = (x2 / scaling) ** 2 + 1.
    metric[2, 2] = (x3 / scaling) ** 2 + 1.
...
def coupled_metric(x1, x2, x3, scaling: float):
    metric = torch.zeros((3, 3))
    metric[0, 0] = (x1 / scaling) ** 2 + 1.
    metric[0, 1] = 0.5 * (x1 / scaling) * (x2 / scaling)
    metric[0, 2] = 0.5 * (x1 / scaling) * (x3 / scaling)

    metric[1, 0] = 0.5 * (x2 / scaling) * (x1 / scaling)
    metric[1, 1] = (x2 / scaling) ** 2 + 1.
    metric[1, 2] = 0.5 * (x2 / scaling) * (x3 / scaling)

    metric[2, 0] = 0.5 * (x3 / scaling) * (x1 / scaling)
    metric[2, 1] = 0.5 * (x3 / scaling) * (x2 / scaling)
    metric[2, 2] = (x3 / scaling) ** 2 + 1.
